...
class HTERRA22MoistureInterpolated:
    ...
    def _form_interpolation_groups(self, band, region, time_period):
        ...
        if region == ht22.CREA_DW and time_period in [ht22.APR_28_AM, ht22.APR_28_PM]:
            return [                
                self._filter_point_subset(df, field_stripes=["CREA_DURUMWHEAT26", "CREA_DURUMWHEAT27"]), # east part, 2 stripes
        ...
        if region == ht22.CAIONE_DW and time_period in [ht22.APR_28_AM, ht22.APR_28_PM]:
            return [
                ...
                self._filter_point_subset(df, field_stripes=["CAIONE1_DURUMWHEAT24", "CAIONE1_DURUMWHEAT27"]), # field 1, south part, 2 stripes
                self._filter_point_subset(df, field_stripes=["CAIONE2_DURUMWHEAT29"]), # field 2, north part, 1 stripe
                self._filter_point_subset(df, field_stripes=["CAIONE2_DURUMWHEAT24", "CAIONE2_DURUMWHEAT27"]), # field 2, south part, 2 stripes
            ]
        if region == ht22.CAIONE_DW and time_period in [ht22.APR_29_AM, ht22.APR_29_PM]:
            return [
                self._filter_point_subset(df, field_stripes=["CAIONE1_DURUMWHEAT24", "CAIONE1_DURUMWHEAT27", "CAIONE1_DURUMWHEAT28"]), # field 1, 3 stripes
                self._filter_point_subset(df, field_stripes=["CAIONE2_DURUMWHEAT24", "CAIONE2_DURUMWHEAT27", "CAIONE2_DURUMWHEAT28"]), # field 2, 3 stripes
            ]
        if region == ht22.CAIONE_MA and time_period in [ht22.JUN_15_AM, ht22.JUN_15_PM, ht22.JUN_16_PM]:
            return [
                self._filter_point_subset(df, field_stripes=["CAIONE_MAIS1", "CAIONE_MAIS2", "CAIONE_MAIS3"]), # west: 3 stripes
    ...
    def load_soil_moisture_lut_raster(self, band, region_name, time_period_id):
        ...
        for sm_points in interpolation_groups:
            if sm_points.shape[0] == 0: continue
            ...
            interpolated_group = scipy.interpolate.griddata(value_coords, soil_moisture, (grid_northing, grid_easting), method="linear").astype(np.float32)
    ...
    def load_soil_moisture_slc_raster(self, band, region_name, time_period_id):
        ...
        for sm_points in interpolation_groups:
            ...
            interpolated_group = scipy.interpolate.griddata(value_coords, soil_moisture, (grid_az, grid_rg), method="linear", rescale=True).astype(np.float32)
            valid_points = np.isfinite(interpolated_group)
